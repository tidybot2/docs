...
First install Phoenix Tuner X on a Windows computer following the [official instructions](https://v6.docs.ctr-electronics.com/en/stable/docs/tuner/index.html).
...
!!! note
    ...
    We recommend connecting the devices in the same order we used.
...
!!! note
    ...
    Once the casters are installed in the frame later, the wires will face inwards and the motor numbers will align with the expected order.
...
!!! note
    ...
    You might want to firmly hold down the module or secure it in place just in case.
...
Start by steering until the encoder position reading is close to 0:

![](images/can-system/Phoenix Tuner X - CANcoder Self Test.png)

Once the encoder position is close to 0, steer the module through one full revolution.
See video (2x speed):
...
Repeat this process for all 4 encoders.
...
After you have verified that all of your motors and encoders are functioning as expected, your drive system should be ready for installation into the frame.
