...
!!! note
    ...
    Leaving an SLA battery in a discharged state for an extended period can degrade its performance and shorten its lifespan.
...
To control the robot in global frame rather than local frame, holding down the right shoulder button instead.
...
1. Press the "Start episode" button to begin teleoperation.
...
!!! tip
    ...
    The numbers on the phone screen show the current min/avg/max/std for the round-trip time (RTT) between the phone and the mini PC.
    ...
    In such cases, we recommend using a [USB-C to Ethernet adapter](https://www.amazon.com/Anker-Ethernet-PowerExpand-Aluminum-Portable/dp/B08CK9X9Z8) to establish a wired Ethernet connection between the phone and the mini PC.
...
To set up data collection, please follow the [phone teleoperation](#phone-teleoperation) section to start the server and connect the phone client.
When starting the server, add the `--save` flag to indicate that episodes should be saved to disk (the default save location is the `data/demos` directory):
...
On the GPU laptop, make sure the `robodiff` Mamba environment is activated, then start the policy server with your desired checkpoint path:

```bash
python policy_server.py --ckpt-path <checkpoint-path>
```

The policy server runs on port `5555` of the GPU laptop.

=== "Real robot"

    On the mini PC, establish an SSH tunnel to the policy server:
    ...
    !!! note
        ...
        The policy server (on the GPU laptop) is different from the Flask server (on the mini PC).

=== "Simulation"

    On your dev machine, establish an SSH tunnel to the policy server:
    ...
    !!! note
        ...
        The policy server (on the GPU laptop) is different from the Flask server (on your dev machine).

Follow these steps to execute policy rollouts:

1. Connect your phone and open the teleoperation web app.
1. Align your phone with the robot.
1. Press the "Start episode" button.
...
!!! note
    ...
    Since the robot is teleoperated during environment resets, your phone still needs to be aligned with the robot when you press the "Start episode" button to ensure proper coordinate frame alignment.
