...
Placing these orders online typically takes about 30 minutes.
...
In addition, there are a few parts that require 3D printing or laser cutting, as well as a single machined part which can be ordered from Xometry.
...
!!! note
    ...
    While it is possible to run everything on one computer, you may need to spend additional effort in mitigating latency spikes in the controller.
...
* **Processor:** The processor does not need to be very powerful since it only needs to run robot controllers. For instance, we have found that the Intel N100 (a modern, energy-efficient entry-level processor) is adequate for low-level control of our robot, and consumes only 20W under load.
