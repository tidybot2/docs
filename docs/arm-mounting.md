...
!!! note
    ...
    Please make sure that the mobile base controller is set up and functioning correctly before mounting an arm, as erratic base movements can damage a mounted arm.
...
=== "Kinova"
    ...
    !!! tip
        ...
        These screws should tighten smoothly but are quite sensitive to misalignment.
        If you find a screw difficult to turn, loosen it and wiggle the mounting plate to correct the alignment.
    ...
    !!! note
        ...
        The E-stop is mounted at an angle against the rails so that it does not move when the button is pressed.
...
=== "Franka"
    ...
    Secure the control box to the top of the frame with 2 rope ratchets:
...
=== "ARX5"
    ...
    Install the arm and [riser](https://github.com/jimmyyhwu/tidybot2-resources/blob/main/3D%20Printing/ARX5/ARX5%20Riser.stl) onto the mounting plate using four M3 100mm screws:
...
=== "UR5"
    ...
    Secure the control box with 2 rope ratchets:
...
=== "ViperX"
    ...
    Adjust the top crossbars so that their centers are 19 cm apart, then install four M5 roll-in T-nuts:
...
Each of our reference designs includes a wrist camera on the arm, equipped with a fisheye lens to increased the field of view.

Begin by installing the fisheye lens onto the wrist camera:
...
=== "Kinova camera"
    ...
    Here are example views from the Kinova camera, both with and without the fisheye lens:
...
=== "Logitech camera"
    ...
    !!! note
        ...
        Make sure the bottom clip is fully inserted into the notch beneath the camera.
        This helps align the fisheye center with the camera center.

    Here are example views from the Logitech camera, both with and without the fisheye lens:
    ...
    !!! note
        ...
        When using the image from the Logitech camera, you might want to crop out the black portions on either side.

=== "Other"

    For other cameras, we provide the CAD file for the fisheye lens mounting ring, which you can modify to design your own fisheye mount:
    ...
    !!! note
        ...
        Design your mount so that the bottom of the mounting ring sits flush with the front surface of the camera.
...
!!! note
    ...
    Since the lens may not be symmetrical, make sure to be consistent if you remove and remount the lens after collecting data.
...
=== "Franka"
    ...
    !!! note
        ...
        The camera mount contains an integrated shim that inserts into the hinge to keep the camera angle fixed.
    ...
    !!! note
        ...
        Since the camera blocks the screw hole, you’ll need to remove the camera to install the mount and then reinsert it afterwards.
...
=== "Other"
    ...
    For other arms, we provide CAD files for a minimal Logitech camera mount, which you can modify to design your own wrist camera mount:
